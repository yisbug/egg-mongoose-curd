...
class OAppWorkerLoader extends egg.AppWorkerLoader {
  ...
  loadSchema() {
    ...
    Object.keys(app.schema).forEach(name => {
      // 添加基础字段
      if (app.config.schema.extendBaseFields) Object.assign(app.schema[name], app.config.schema.baseFields);
      // 扩展字段
      ...
      if (app.config.schema.service) {
        ...
        if (customService) Object.assign(this.app.serviceClasses[name].prototype, customService);
      }
      // 不支持覆盖controller
...
module.exports = Object.assign(egg, {
  ...
  AppWorkerLoader: OAppWorkerLoader,
});
